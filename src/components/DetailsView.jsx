...
const DetailsView = ({
    activeSheet, headers, data, newRow,
    onInputChange, onSubmit, onRefresh,
            ...
            <main style={styles.main}>
                <header style={styles.topBar}>
                    <div>
                        ...
                        <p style={styles.pageSubtitle}>{data.length} records · {headers.length} columns</p>
                    </div>
                    <div style={styles.topActions}>
                        <button className="dv-refresh" style={styles.btnRefresh} onClick={onRefresh}>
                            ↻ Refresh
                        </button>
                        <button
                            className="dv-add"
                            style={{ ...styles.btnAdd, ...(showForm ? styles.btnCancel : {}) }}
                ...
                <div style={styles.contentStack}>
                    {showForm && (
                        <div style={styles.formPanel}>
                            ...
                            <form onSubmit={(e) => { onSubmit(e); setShowForm(false); }} style={styles.formGrid}>
                                {headers.map(header => (
                    ...
                    <div style={{ ...styles.tableCard, marginTop: showForm ? '-16px' : 0 }}>
                        {loading ? (
                                    ...
                                    <tbody>
                                        {data.map((row, i) => (
                                            <tr key={i} className="dv-tr">
                                                <td style={{ ...styles.td, ...styles.tdIdx }}>{i + 1}</td>
                                                {headers.map(h => (
